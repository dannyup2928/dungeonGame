...
uint32_t throw_granade(dungeon *d)

{
  ...
  * not otherwise used.                                                 */
  ...
  mvprintw(1,0, "you bomb %s. %s is dead", charpair(dest)->name, charpair(dest)->name);
...
uint32_t io_shop(dungeon *d)
{
  uint32_t i, key;
  char s[61];

 mvprintw(3,10, "esc to cancle, 0-5 to buy ");

attron(COLOR_PAIR(COLOR_RED));

  mvprintw(4,10, "                  Shop ");
attroff(COLOR_PAIR(COLOR_RED));
  mvprintw(5,10, "Money you have:  $%4d dollars", d->PC->money);
  mvprintw(6,10, " ");

  for (i = 0; i < 3; i++) {
   
    io_object_to_string(d->PC->shopItems[i], s, 61);
    mvprintw(i + 7, 10, " %c) $1000  %-55s ", '0' + i, s);
  }

    mvprintw(10, 10, " %c) $5000 level up potion (level up to increase speed, hp and)", '3');


    mvprintw(11, 10, " %c) $2000 granade (throw within a radius to kill a monster)", '4');

    mvprintw(12, 10, " %c) $9999  Atomic Bomb (sacrifice yourself and save the world) ", '5');
  ...
  while (1) {
    ...
    if (key < '0' || key > '5') {
      if (isprint(key)) {
        snprintf(s, 61, "Invalid input: '%c'.  Enter 0-5 or ESC to cancel.",
                 key);
        mvprintw(18, 10, " %-58s ", s);
      } else {
        mvprintw(18, 10, " %-58s ",
                 "Invalid input.  Enter 0-5 or ESC to cancel.");
      }
      refresh();
      continue;
    }
    
    if(key == '5' && d->PC->money >= 9999){
      d->PC->sacrifice= true;
      d->PC->money -= 9999;
      return 0;
}
    if(key == '5' && d->PC->money < 9999){ 
     mvprintw(18, 10, " %-58s ", "not enough money");
     continue;
}

    if(key == '4' && d->PC->money >= 2000){
      throw_granade(d);
      d->PC->money -= 2000;
      return 0;
}
    ...
    if (!d->PC->shopItems[key - '0']) {
      mvprintw(18, 10, " %-58s ", "Items sold.");
      continue;
    }

    if (d->PC->shopItems[key - '0'] && d->PC->money <1000) {
      mvprintw(18, 10, " %-58s ", "not enough money");
      continue;
    }


      if (!d->PC->buy_item(key - '0')) {
      d->PC->money -=1000; 
      return 0;
    }


    snprintf(s, 61, "Can't buy %s.  check your inventroy.",
             d->PC->shopItems[key - '0']->get_name());
    mvprintw(18, 10, " %-58s ", s);
    refresh();
  }
