...
uint32_t pc::buy_item(uint32_t slot)
{


  if (!shopItems[slot]                      ||
      !shopItems[slot]->is_removable() ||
      !has_open_inventory_slot()) {
    io_queue_message("You can't buy %s, because you have nowhere to put it.",
                     shopItems[slot]->get_name());

    return 1;
  }


  io_queue_message("You buy %s.", shopItems[slot]->get_name());
